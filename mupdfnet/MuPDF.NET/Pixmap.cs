...
namespace MuPDF.NET
{
    public class Pixmap
    {
        ...
        public object/*dynamic*/ ColorCount(bool colors = false, object/*dynamic*/ clip = null)
        {
            FzPixmap pm = _nativePixmap;
        ...
        /// <returns></returns>
        public ValueTuple<float, byte[]> ColorTopUsage(object/*dynamic*/ clip = null)
        {
            int allPixels = 0;
            int count = 0;
            ...
            if (allPixels == 0)
                return new ValueTuple<float, byte[]>(1, Enumerable.Repeat((byte)255, N).ToArray());
            return new ValueTuple<float, byte[]>(count / (float)allPixels, maxPixel.Split(',').Select(b => byte.Parse(b)).ToArray());
        }
        ...
        /// <exception cref="Exception"></exception>
        public void SetAlpha(object/*dynamic*/ alphaValues = null, int premultiply = 1, object/*dynamic*/ opaque = null, object/*dynamic*/ matte = null)
        {
            FzPixmap pixmap = _nativePixmap;
            int alpha = 0;
            int m = 0;
            ...
            int zeroOut = 0;
            int bground = 0;
            if (opaque != null && (opaque is List<int>) && ((List<int>)opaque).Count == n)
            {
                foreach (int i in (List<int>)opaque)
                {
                	colors[i] = ((List<int>)opaque)[i];
                }
                zeroOut = 1;
            } 
#if flase 
            if (opaque != null && (opaque is Tuple) && ((Tuple)opaque).Count == n)
            {           	
                foreach (int i in (Tuple)opaque)
                {
                	colors[i] = ((Tuple)opaque)[i];
                }
                zeroOut = 1;
            }
#endif                
...
#if false            
            if (matte != null && (matte is Tuple) && matte.Count == n)
            {
...
#endif            
			if (matte != null && (matte is List<int>) && ((List<int>)matte).Count == n)
            {
				foreach (int i in (List<int>)matte)
					bgColor[i] = ((List<int>)matte)[i];
                bground = 1;
            }
            ...
            if (alphaValues != null)
            {
                if (alphaValues is List<byte>)
                {
                	data = new List<byte>((List<byte>)alphaValues);
                	dataLen = ((List<byte>)alphaValues).Count();
                }
                else if (alphaValues is byte[])
                {
                	data = new List<byte>((byte[])alphaValues);
                	dataLen = ((byte[])alphaValues).Count();
                }
                else
