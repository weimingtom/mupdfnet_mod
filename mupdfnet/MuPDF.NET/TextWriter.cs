...
namespace MuPDF.NET
{
    public class TextWriter
    {
        ...
        public ValueTuple<Rect, Point> Append(Point pos, string text, Font font, float fontSize = 11.0f, string language = null, int right2left = 0, int smallCaps = 0)
        {
            pos = pos * ICtm;
            if (font == null)
            {
            ...
            TextRect = Bbox * Ctm;
            ValueTuple<Rect, Point> ret = new ValueTuple<Rect, Point>(TextRect, LastPoint);
        ...
        /// <param name="smallCaps"></param>
        /// <returns></returns>
        public ValueTuple<Rect, Point> Appendv(Point pos, string text, Font font = null, float fontSiz = 11.0f,
            string language = null, bool smallCaps = false)
        {
            ...
            return new ValueTuple<Rect, Point>(TextRect, LastPoint);
        }
            ...
            Matrix matrix = null, int renderMode = 0, int oc = 0)
        {
            ...
            ValueTuple<int, int> maxNums = Utils.MergeResources(pdfPage, resources);
            string cont = Utils.EscapeStrFromBuffer(contents);
            int maxAlp = maxNums.Item1;
            int maxFont = maxNums.Item2;
            ...
            float TextLen(string x, Font font, float fontSize, bool smallCaps)
            {
                return font.TextLength(x, fontSize: fontSize, smallCaps: smallCaps ? 1 : 0);
            }

            List<float> CharLengths(string x, Font font, float fontSize, bool smallCaps)
            {
                return font.GetCharLengths(x, fontSize: fontSize, smallCaps: smallCaps ? 1 : 0);
            }

            ValueTuple<Rect, Point> AppendThis(Point pos, string text, Font font, float fontSize, bool smallCaps)
            {
            ...
            ValueTuple<List<string>, List<float>> NormWords(float width, List<string> words, Font font, float fontSize, bool smallCaps)
            {
                List<string> nwords = new List<string>();
                List<float> wordLengths = new List<float>();
                foreach (string word in words)
                {
                    List<float> charLengths = CharLengths(word, font, fontSize, smallCaps);
                ...
                return new ValueTuple<List<string>, List<float>>(nwords, wordLengths);
            }

            void OutputJustify(Point start, string line, float stdWidth, Font font, float fontSize, bool smallCaps)
            {
                ...
                if (nwords == 1)
                {
                    AppendThis(start, words[0], font, fontSize, smallCaps);
                    return;
                }
                float tl = words.Sum(x => TextLen(x, font, fontSize, smallCaps));
                int gaps = nwords - 1;
                float gapl = (stdWidth - tl) / gaps;
                foreach (string w in words)
                {
                    ValueTuple<Rect, Point> __ = AppendThis(start, w, font, fontSize, smallCaps);
                    Rect _ = __.Item1;
                    Point lp = __.Item2;
                    start.X = lp.X + gapl;
                }
        ...
        /// <exception cref="Exception"></exception>
        public List<ValueTuple<string, float>> FillTextbox(
            Rect rect,
            string text,
            Font font,
            Point pos = null,
            ...
            bool smallCaps = false)
        {
            ...
            float spaceLen = TextLen(" ", font, fontSize, smallCaps);
            ...
            List<ValueTuple<string, float>> newLines = new List<ValueTuple<string, float>>();
            ...
            foreach (string line in textLines)
            {
                ...
                if (line == "" || line == " ")
                {
                    newLines.Add(new ValueTuple<string, float>(line, spaceLen));
                ...
                float tl = TextLen(line_, font, fontSize, smallCaps);
                if (tl <= width)
                {
                    newLines.Add(new ValueTuple<string, float>(line, tl));
                    noJustify.Add((newLines.Count - 1));
                    continue;
                }

                string[] words = line.Split(' ');
                ValueTuple<List<string>, List<float>> __ = NormWords(stdWidth, new List<string>(words), font, fontSize, smallCaps);
                List<string> words_ = __.Item1;
                List<float> wordLengths_ = __.Item2;
                ...
                while (true)
                {
                    ...
                    if (wl <= width)
                    {
                        newLines.Add(new ValueTuple<string, float>(line0, wl));
            ...
            for (i = 0; i < maxLines; i++)
            {
                ...
                try
                {
                    ValueTuple<string, float> _ = newLines[0];
                    line = _.Item1;
                    tl = _.Item2;
                    newLines.RemoveAt(0);
                }
                ...
                {
                    OutputJustify(start, line, stdWidth, font, fontSize, smallCaps);
                    start.X = stdStart;
                    start.Y += LineHeight;
                    continue;
                }
                ...
                AppendThis(start, line, font, fontSize, smallCaps);
                start.X = stdStart;
                start.Y += LineHeight;
            }
            return newLines;
