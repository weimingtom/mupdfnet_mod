...
namespace MuPDF.NET
{
    public class Page
    {
        ...
        public Dictionary<int, object/*dynamic*/> AnnotRefs { get { return AnnotRefs_;}  set { AnnotRefs_ = value; } }
        private Dictionary<int, object/*dynamic*/> AnnotRefs_ = new Dictionary<int, dynamic>();
        ...
        public static ValueTuple<Rect, Rect, Matrix> RectFunction(int rectN, Rect filled)
        {
            return new ValueTuple<Rect, Rect, Matrix>(fit.Rect, fit.Rect, new IdentityMatrix());
        }

        public FzPage AsFzPage(object/*dynamic*/ page)
        {
            if (page is Page)
        ...
        public Annot AddFileAnnot(
            ...
            byte[] buffer_,
            string filename,
            object/*dynamic*/ ufilename = null,
            string desc = null,
            string icon = null
        )
        {
            PdfPage page = _pdfPage;
            string uf = ufilename != null ? (string)ufilename : filename;
        ...
        )
        {
            ...
            List<Block> blocks = ((PageInfo)GetText("dict", clip, 0)).Blocks; //BLOCKS
        ...
        private List<ValueTuple<string, int>> GetResourceProperties()
        {
            PdfPage page = _pdfPage;
            List<ValueTuple<string, int>> rc = Utils.GetResourceProperties(page.obj());
        ...
        )
        {
            ...
            if (do_have_imask != 0)
            {
                ...
                ValueTuple<int, int> _ = image.fz_image_resolution();
                int xres = _.Item1;
                int yres = _.Item2;
        ...
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        public ValueTuple<float, float> InsertHtmlBox(
            Rect rect,
            object/*dynamic*/ text,
            string css = null,
            float opacity = 1,
        ...
        )
        {
            ...
            Story story = null;
            if (text is string)
            	story = new Story((string)text, mycss, archive: archive);
            ...
            if (fit.BigEnough == false)
                return new ValueTuple<float, float>(-1, scaleLow);

            var filled = fit.Filled;
            float scale = 1 / fit.Parameter;

            float spareHeight = fit.Rect.Y1 - ((Rect)filled)[3];
            ...
            return new ValueTuple<float, float>(spareHeight, scale);
        }
        ...
        )
        {
            ...
            int xref = 0;
            if (doc.ShownPages.TryGetValue(new ValueTuple<int, int>(isrc, pno), out xref))
            {
            ...
            doc.ShownPages[new ValueTuple<int, int>(isrc, pno)] = xref;
        ...
        public string GetOptionalContent(int oc)
        {
            ...
            List<ValueTuple<string, int>> props = GetResourceProperties();
            foreach (ValueTuple<string, int> _ in props)
            {
            	string p = _.Item1;
            	int x = _.Item2;
                propsDict[x] = p;
        ...
        public Annot DeleteAnnot(Annot annot)
        {
        	...
        	void Finished(LinkInfo link)
            {
                ...
                try
                {
                    ...
                    Annot linkObj = (Annot)AnnotRefs[0]; // MuPDFAnnotation or Link, Widget
                    ((Annot)linkObj).Erase();
                }
                catch (Exception)
                {
        ...
        public void GetImageBbox(string name, bool transform = false)
        {
            ...
            ValueTuple<Rect, Matrix> rc;
            if (transform)
                rc = new ValueTuple<Rect, Matrix>(infRect, nullMat);
        }
        ...
        {
            List<ValueTuple<int, string>> labels = Parent._getPageLabels();
            if (labels.Count == 0)
                return "";
            labels.Sort();
        ...
        public string GetTextbox(Rect rect = null, TextPage textPage = null)
        {
        	...
        	TextPage FullOcr(Page page, int dpi, string language, int flags, string tessdata)
            {
        ...
        )
        {
            ...
            if (full)
                return FullOcr(this, dpi, language, flags, tessdata);
        ...
        public Annot AddWidget(Widget widget)
        {
        	...
        	bool AreNeighbors(Rect r1, Rect r2, float deltaX, float deltaY)
            {
                float rr1_X0,
                    rr1_X1,
                    rr1_Y0,
                    ...
                    rr2_Y0,
                    rr2_Y1;
                rr1_X0 = (r1.X1 > r1.X0) ? r1.X0 : r1.X1;
                rr1_X1 = (r1.X1 > r1.X0) ? r1.X1 : r1.X0;
                rr1_Y0 = (r1.Y1 > r1.Y0) ? r1.Y0 : r1.Y1;
                rr1_Y1 = (r1.Y1 > r1.Y0) ? r1.Y1 : r1.Y0;
                rr2_X0 = (r2.X1 > r2.X0) ? r2.X0 : r2.X1;
                rr2_X1 = (r2.X1 > r2.X0) ? r2.X1 : r2.X0;
                rr2_Y0 = (r2.Y1 > r2.Y0) ? r2.Y0 : r2.Y1;
				rr2_Y1 = (r2.Y1 > r2.Y0) ? r2.Y1 : r2.Y0;                
