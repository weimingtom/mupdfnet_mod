...
namespace MuPDF.NET
{
    public class Widget
    {
        ...
        public List<object/*dynamic*/> ChoiceValues { get; set; }
        ...
        public void Checker()
        {
            ...
            if (FieldType == (int)PdfWidgetType.PDF_WIDGET_TYPE_RADIOBUTTON)
            {
                ...
                ValueTuple<string, string> _ = doc.GetKeyXref(Xref, "Parent/Kids");
                string kidsType = _.Item1;
                string kidsValue = _.Item2;
                if (kidsType == "array")
                {
                	List<int> xrefs = kidsValue.Substring(1, kidsValue.Length - 2).Replace("0 R", "").Split(new string[] {""}, StringSplitOptions.None).Select(x => int.Parse(x)).ToList();
        ...
        public void ParseDa()
        {
            ...
            float fontSize = 0;
            float[] col = { 0, 0, 0 };
            string[] dat = TextDa.Split(new string[] {""}, StringSplitOptions.None);
            int i = 0;
        ...
        public Dictionary<string, List<string>> ButtonStates()
        {
            ...
            ValueTuple<string, string> apn = doc.GetKeyXref(xref, "AP/N");
            if (apn.Item1 == "dict")
            {
            ...
            ValueTuple<string, string> apd = doc.GetKeyXref(xref, "AP/D");
            if (apd.Item1 == "dict")
            {
        ...
        public Annot Next
        {
            get
            {
            	return (Annot)(new Annot(_annot)).Next;
            }
        }
