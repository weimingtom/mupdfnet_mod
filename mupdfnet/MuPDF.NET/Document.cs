...
namespace MuPDF.NET
{
    public class Document
    {
        ...
        public Dictionary<int, GraftMap> GraftMaps { get { return GraftMaps_;} set {GraftMaps_ = value;} }
        private Dictionary<int, GraftMap> GraftMaps_ = new Dictionary<int, GraftMap>();

        public Dictionary<ValueTuple<int, int>, int> ShownPages { get { return ShownPages_;} set { ShownPages_ = value;} }
        private Dictionary<ValueTuple<int, int>, int> ShownPages_ = new Dictionary<ValueTuple<int, int>, int>();

        public Dictionary<string, int> InsertedImages { get { return InsertedImages_;} set { InsertedImages_ = value;} }
        private Dictionary<string, int> InsertedImages_ = new Dictionary<string, int>();
        ...
        /// </summary>
        public ValueTuple<int, int> LastLocation
        {
            get
            {
                if (IsClosed)
                ...
                return new ValueTuple<int, int>(lastLoc.chapter, lastLoc.page);
            }
        }
        ...
        public string PageLayout
        {
            get
            {
                ...
                if (xref == 0)
                    return null;
                ValueTuple<string, string> rc = GetKeyXref(xref, "PageLayout");
                if (rc.Item1 == "null")
                    return "SinglePage";
                if (rc.Item1 == "name")
        ...
        public string PageMode
        {
            get
            {
                ...
                if (xref == 0)
                    return null;
                ValueTuple<string, string> rc = GetKeyXref(xref, "PageMode");
                if (rc.Item1 == "null")
                    return "UseNone";
                if (rc.Item1 == "name")
                    return rc.Item2.Substring(1);
        ...
        public Dictionary<string, bool> MarkInfo
        {
            get
            {
                ...
                if (xref == 0)
                    return null;
                ValueTuple<string, string> rc = GetKeyXref(xref, "MarkInfo");
                if (rc.Item1 == "null")
...
/*                                        if (
        ...
        public void InitDocument()
        {
            ...
            foreach (KeyValuePair<string, string> _ in values)
            {
            	string key = _.Key;
        ...
        public ValueTuple<string, string> GetKeyXref(int xref, string key)
        {
            if (IsClosed)
                throw new Exception("document closed");
            ...
            return new ValueTuple<string, string>(type, text);
        }

        public void Save(
            object/*dynamic*/ filename,
            int garbage = 0,
            int clean = 0,
            int deflate = 0,
            int deflateImages = 0,
        ...
        )
        {
            ...
            {
            	pdf.pdf_save_document((string)filename, opts);
            }
            else
            {
            	output = new FilePtrOutput((MemoryStream)filename);
        ...
        public Page ReloadPage(Page page)
        {
            ...
            int pno = page.Number;
            foreach (KeyValuePair<int, object> _ in page.AnnotRefs)
            {
            	int k = _.Key;
            	object/*dynamic*/ v = _.Value;
                oldAnnots.Add(k, v);
            }
            ...
            foreach (KeyValuePair<int, object> _ in oldAnnots)
            {
            	int k = _.Key;
            	object/*dynamic*/ v = _.Value;
                object/*dynamic*/ annot = oldAnnots[k];
        ...
        public List<ValueTuple<int, double>> _GetCharWidths(
            int xref,
            string bfName,
            string ext,
            int ordering,
        ...
        )
        {
            ...
            List<ValueTuple<int, double>> wList = new List<ValueTuple<int, double>>();
            for (int i = 0; i < myLimit; i++)
            {
                ...
                if (ordering >= 0)
                    glyph = i;
                if (glyph > 0)
                    wList.Add(new ValueTuple<int, double>(glyph, adv));
                else
                    wList.Add(new ValueTuple<int, double>(glyph, 0.0f));
            }
        ...
        private void _DeletePage(int pno)
        {
        	...
        	List<Toc> Recurse(Outline olItem, List<Toc> list, int lvl, bool simple)
            {
                while (olItem != null && olItem.ToFzOutline().m_internal != null)
                {
                    ...
                    if (!olItem.IsExternal)
                    {
                        if (olItem.Uri != null)
                        {
                            if (olItem.Page == -1)
                            {
                                ValueTuple<List<int>, float, float> resolve = ResolveLink(olItem.Uri);
                    ...
                    if (olItem.Down != null)
                        list = Recurse(olItem.Down, list, lvl + 1, simple);
                    olItem = olItem.Next;
                }
        ...
        public List<Toc> GetToc(bool simple = true)
        {
            ...
            List<Toc> toc = Recurse(olItem, liste, lvl, simple);
        ...
        /// <returns></returns>
        internal ValueTuple<List<int>, float, float> ResolveLink(string uri = null, int chapters = 0)
        {
            ...
            {
                if (chapters != 0)
                    return new ValueTuple<List<int>, float, float>(new List<int>() { -1, -1 }, 0, 0);
                return new ValueTuple<List<int>, float, float>(new List<int>() { -1 }, 0, 0);
            }
            try
            {
            ...
            if (chapters != 0)
                return new ValueTuple<List<int>, float, float>(new List<int>() { loc.chapter, loc.page }, xp, yp);
            int pno = _nativeDocument.fz_page_number_from_location(new FzLocation(loc));
            return new ValueTuple<List<int>, float, float>(new List<int>() { pno }, xp, yp);
        }
        ...
        public Dictionary<string, dynamic> ResolveNames()
        {
            ...
            Dictionary<string, object/*dynamic*/> ret = new Dictionary<string, object/*dynamic*/>();
            foreach (KeyValuePair<string, DestName> _ in destDict)
            {
            	string k = _.Key;
            	DestName v = _.Value;
                ret.Add(k, v);
        ...
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        public ValueTuple<bool, bool> JournalCanDo()
        {
            if (IsClosed || IsEncrypted)
                throw new Exception("document closed or encrypted");
            int undo = 0;
            ...
            return new ValueTuple<bool, bool>(undo != 0, redo != 0);
        }
        ...
        /// </summary>
        /// <exception cref="Exception"></exception>
        public ValueTuple<int, int> JournalPosition()
        {
            if (IsClosed || IsEncrypted)
                throw new Exception("document closed or encrypted");
            PdfDocument pdf = Document.AsPdfDocument(_nativeDocument);
            ValueTuple<int, int> rc = pdf.pdf_undoredo_state();
        ...
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        public ValueTuple<int, int> GetLocationFromPageNumber(int pno)
        {
            if (IsClosed)
            {
                throw new Exception("document is closed");
            ...
            return new ValueTuple<int, int>(loc.chapter, loc.page);
        }
        ...
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        public ulong MakeBookmark(ValueTuple<int, int> locNumbers)
        {
            if (IsClosed || IsEncrypted)
                throw new Exception("document closed or encrypted");
        ...
        private void MoveCopyPage(int pno, int nb, bool before, bool copy)
        {
            PdfDocument pdf = AsPdfDocument(_nativeDocument);
            bool same;
            ValueTuple<PdfObj, PdfObj, int> _ = pdf.pdf_lookup_page_loc(pno);
            PdfObj page1 = _.Item1;
            PdfObj parent1 = _.Item2;
            int i1 = _.Item3;
            PdfObj kids1 = parent1.pdf_dict_get(new PdfObj("Kids"));

            _ = pdf.pdf_lookup_page_loc(nb);
            PdfObj page2 = _.Item1;
            PdfObj parent2 = _.Item2;
            int i2 = _.Item3;
        ...
        /// <exception cref="Exception"></exception>
        public ValueTuple<int, int> NextLocation(int pageId)
        {
            if (IsClosed || IsEncrypted)
                throw new Exception("document closed or encrypted");
            ValueTuple<int, int> _pageId;
            _pageId = new ValueTuple<int, int>(0, pageId);
            ...
            return new ValueTuple<int, int>(nextLoc.chapter, nextLoc.page);
        }
        ...
        /// <returns></returns>
        public ValueTuple<int, int> NextLocation(ValueTuple<int, int> pageId)
        {
            if (IsClosed || IsEncrypted)
                throw new Exception("document closed or encrypted");
        ...
        public int GetPageNumberFromLocation(int pageId)
        {
            ...
            while (pageId < 0)
                pageId += pageN;
            ValueTuple<int, int> _pageId = new ValueTuple<int, int>(0, pageId);
            if (!Contains(_pageId))
                throw new Exception("page id not in document");

            ValueTuple<int, int> _ = _pageId;
            int chapter = _.Item1;
            int pno = _.Item2;
            FzLocation loc = mupdf.mupdf.fz_make_location(chapter, pno);
        ...
        internal List<ValueTuple<int, string>> _getPageLabels()
        {
            PdfDocument pdf = AsPdfDocument(this);
            List<ValueTuple<int, string>> rc = new List<ValueTuple<int, string>>();
        ...
        public List<Label> GetPageLabels()
        {
            ...
            foreach (ValueTuple<int, string> item in _getPageLabels())
            {
        ...
        public int EmbeddedfileIndex(object/*dynamic*/ item)
        {
            List<string> filenames = GetEmbfileNames();
            ...
            if (item is string && filenames.Contains(item))
            	idx = filenames.IndexOf((string)item);
            else if (item is int && Utils.INRANGE((int)item, 0, filenames.Count - 1))
            	idx = (int)item;
            else
                throw new Exception(msg);
            return idx;
        ...
        public EmbfileInfo GetEmbfileInfo(object/*dynamic*/ item)
        {
            ...
            infoDict.Size = DL;
            infoDict.Length = len;
            ValueTuple<string, string> _ = GetKeyXref(xref, "Params/CreationDate");
            string t = _.Item1;
            string date = _.Item2;
            if (t != "null")
                infoDict.ModDate = date;
            _ = GetKeyXref(xref, "Params/ModDate");
            t = _.Item1;
            string md5 = _.Item2;
            if (t != "null")
            {
        ...
        public ImageInfo ExtractImage(int xref)
        {
            ...
            ValueTuple<int, int> _ = img.fz_image_resolution();
            float xres = _.Item1;
            float yres = _.Item2;
            float width = img.w();
            float height = img.h();
        ...
        public void SetLayerUIConfig(object/*dynamic*/ number, int action = 0)
        {
            int num;
            if (number is string)
            {
        ...
        public bool SetMarkInfo(Dictionary<string, bool> markInfo)
        {
            ...
            string pdfDict = "<<";
            foreach (KeyValuePair<string, bool> _ in valid)
            {
            	string k = _.Key;
            	bool v = _.Value;
        ...
        public void CopyXref(int newXref, int xref, List<string> keep = null)
        {
            ...
            {
                ValueTuple<string, string> item = GetKeyXref(newXref, key);
        ...
        public void SetMetadata(Dictionary<string, string> metadata = null)
        {
            ...
            ValueTuple<string, string> _ = GetKeyXref(-1, "Info");
            string t = _.Item1;
            string temp = _.Item2;
            int infoXref = 0;
            if (t != "xref")
            ...
            {
            	string result = null;
            	if (keymap.TryGetValue(k, out result))
            	{
        ...
        public void SetOC(int xref, int oc)
        {
            ...
            ValueTuple<string, string> _ = GetKeyXref(xref, "Subtype");
            string t = _.Item1;
            string name = _.Item2;
            ...
            if (oc > 0)
            {
                ValueTuple<string, string> __ = GetKeyXref(oc, "Type");
                t = __.Item1;
                name = __.Item2;
            }
        	...
        	string VeMaker(object/*dynamic*/[] v, List<int> allOcgs)
            {
                if (v.Length < 2)
                    throw new Exception("bad ve length: " + v.Length + "");
                if (
                    !(v[0] is string)
                    || !(new List<string>() { "and", "or", "not" }).Contains(((string)v[0]).ToLower())
                )
                    throw new Exception("bad operand: " + v[0] + "");
                if (((string)v[0]).ToLower() == "not" && v.Length != 2)
                ...
                foreach (var x in v.Skip(1).ToArray())
                {
                    if (x is int)
                    {
                    	if (!allOcgs.Contains((int)x))
                            throw new Exception("bad OCG " + x + "");
                        item += " " + x + " 0 R";
                    }
                    else
                    {
                    	item += " " + VeMaker((object[])x, allOcgs) + "";
                    }
                }
                item += "]";
                return item;
        ...
        public int SetToc(List<Toc> tocs, int collapse = 1)
        {
            ...
            List<Dictionary<string, object/*dynamic*/>> olItems = new List<Dictionary<string, object/*dynamic*/>>()
            {
                new Dictionary<string, object/*dynamic*/>()
                {
                    { "count", 0 },
                    { "first", -1 },
                    { "last", -1 },
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (o.Link != null)
                {
                    ...
                    {
                    	dest.To = new Point(72, pageHeight - (float)o.Link);
                    }
                }

                Dictionary<string, object/*dynamic*/> d = new Dictionary<string, object/*dynamic*/>();
                d.Add("first", -1);
                d.Add("count", 0);
                d.Add("last", -1);
                d.Add("prev", -1);
                ...
                lvlTab[lvl] = i + 1;
                Dictionary<string, object/*dynamic*/> parent = olItems[lvlTab[lvl - 1]];

                if (dest.Collapse || (collapse != 0 && lvl > collapse))
                	parent["count"] = (int)parent["count"] - 1;
                else
                	parent["count"] = (int)parent["count"] + 1;

                if ((int)parent["first"] == -1)
                {
                    parent["first"] = i + 1;
                    parent["last"] = i + 1;
                }
                ...
                {
                    d["prev"] = parent["last"];
                    Dictionary<string, object/*dynamic*/> prev = olItems[(int)parent["last"]];
                    prev["next"] = i + 1;
                    parent["last"] = i + 1;
                }
                olItems.Add(d);
            ...
            int index = 0;
            foreach (Dictionary<string, object/*dynamic*/> ol in olItems)
            {
                string txt = "<<";
                if ((int)ol["count"] != 0)
                {
                ...
                {
                    if ((int)ol["first"] > -1)
                    	txt += "/First " + xref[(int)ol["first"]] + " 0 R";
                }
                catch (Exception) {  }
                ...
                {
                    if ((int)ol["parent"] > -1)
                        txt += "/Parent " + xref[(int)ol["parent"]] + " 0 R";
                }
                catch (Exception) { }
                ...
                object result1_ = null;
                int result1 = 0;
                if (ol.TryGetValue("count", out result1_))
                {
                	if (result1_ != null && result1_ is int)
                	{
                		result1 = (int)result1_;
                	}
                	else
                	{
                		result1 = 0;
                ...
                object result2 = 0;
                if (ol.TryGetValue("color", out result2))
                {
                ...
                {
                	if ( ((int[])ol["color"]).Length == 3)
                    {
                    	txt += "/C[ " + ((int[])ol["color"])[0] + " " + ((int[])ol["color"])[1] + " " + ((int[])ol["color"])[2] + "]";
                    }
                }
                ...
                int _result = 0;
                if (ol.TryGetValue("flags", out _result2))
                {
                	if (_result2 != null)
                	{
                		_result = (int)_result2;
                	}
                	else
                	{
                		_result = 0;
        ...
        public bool Contains(ValueTuple<int, int> loc)
        {
            ValueTuple<int, int> _ = loc;
            int chapter = _.Item1;
            int pno = _.Item2;
        ...
        public ValueTuple<int, int> PrevLocation(int pno, int chapter = 0)
        {
            if (IsClosed || IsEncrypted)
                throw new Exception("document closed or encrypted");
            if (pno == 0 && chapter == 0)
            ...
            return new ValueTuple<int, int>(prevLoc.chapter, prevLoc.page);
        }
    }
}
