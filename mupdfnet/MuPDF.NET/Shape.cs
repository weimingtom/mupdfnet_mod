...
namespace MuPDF.NET
{
    public class Shape
    {
            ...
            )
        {
            ...
            List<ValueTuple<int, double>> g = null;
            foreach (string t in text)
            {
                if (simple && !(bfName == "Symbol" || bfName == "ZapfDingbats"))
                    g = null;
                else
                    g = new List<ValueTuple<int, double>>(glyphs);
            ...
            )
        {
        	...
        	float PixLen(string x, float fontSize, int ordering, List<ValueTuple<int, double>> glyphs)
            {
                if (ordering < 0)
                {
                    double sum = 0;
            ...
            )
        {
            ...
            int ordering = fontInfo.Ordering;
            bool simple = fontInfo.Simple;
            List<ValueTuple<int, double>> glyphs = fontInfo.Glyphs;
            string bfName = fontInfo.Name;
            ...
            List<ValueTuple<int, double>> tj_glyphs;
            if (simple && !(bfName == "Symbol" || bfName == "ZapfDingbats"))
                tj_glyphs = null;
            else
                tj_glyphs = glyphs;
            ...
            for (int i = 0; i < t2.Length; i++)
            {
                ...
                {
                    float pl_w = PixLen(word, fontSize, ordering, glyphs);
                    if (rest >= pl_w)
                    {
                        lbuff += word + " ";
                        rest -= (pl_w + blen);
                    ...
                    {
                        if (PixLen(lbuff, fontSize, ordering, glyphs) <= (maxWidth - PixLen(Convert.ToString(c), fontSize, ordering, glyphs)))
                            lbuff += c;
                        else
                        {
                            lbuff += "\n";
                    ...
                    lbuff += " ";
                    rest = maxWidth - PixLen(lbuff, fontSize, ordering, glyphs);
                }
                if (!string.IsNullOrEmpty(lbuff))
                {
                    text += lbuff.TrimEnd();
            ...
            {
                float pl = maxWidth - PixLen(text_t[i], fontSize, ordering, glyphs);
