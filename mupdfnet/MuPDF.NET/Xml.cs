...
namespace MuPDF.NET
{
    public class Xml
    {
        ...
        private List<ValueTuple<int, string>> ShowNode(Xml node, List<ValueTuple<int, string>> items, int shift)
        {
            while (node != null)
            {
                if (node.IsText)
                {
                    items.Add(new ValueTuple<int, string>(shift, "\"" + node.Text + "\""));
                    node = node.Next;
                    continue;
                }
                items.Add(new ValueTuple<int, string>(shift, "\"" + node.TagName + "\""));
                foreach (KeyValuePair<string, string> _ in node.GetAttributes()) 
                {
                	string k = _.Key; 
                	string v = _.Value;
                    items.Add(new ValueTuple<int, string>(shift, "=" + k + " \"" + v + "\""));
                }
        ...
        private List<ValueTuple<int, string>> GetNodeTree()
        {
            int shift = 0;
        ...
        public Xml AddHeader(int level = 1)
        {
            ...
            if (!(new List<String>(new[] { "h1", "h2", "h3", "h4", "h5", "h6" })).Contains(tagName))
            {
                AppendChild(child);
                return child;
            }
        ...
        public static string ColorText(object color)
        {
            ...
            if (color is int)
            {
            	ValueTuple<int, int, int> rgb = Utils.sRGB2rgb((int)color);
                return "rgb(" + rgb.Item1 + "," + rgb.Item2 + "," + rgb.Item3 + ")";
            }
            if ((color is float[] && ((float[])color).Length == 3))
            {
            	return "rbg(" + ((float[])color)[0] + "," + ((float[])color)[1] + "," + ((float[])color)[2] + "";
            }
            return (string)color;
        }
        ...
        public void Debug()
        {
            List<ValueTuple<int, string>> items = GetNodeTree();
            foreach (ValueTuple<int, string> _ in items)
            {
            	int k = _.Item1;
            	string v = _.Item2;
        ...
        public Xml SetAlign(object align)
        {
            ...
            if (align is string)
            	text += (string)align;
            else if (align is int && (int)align == Utils.TEXT_ALIGN_LEFT)
                text += "left";
            else if (align is int && (int)align == Utils.TEXT_ALIGN_CENTER)
                text += "center";
            else if (align is int && (int)align == Utils.TEXT_ALIGN_RIGHT)
                text += "right";
            else if (align is int && (int)align == Utils.TEXT_ALIGN_JUSTIFY)
                text += "justify";
            else
        ...
        /// <returns></returns>
        public Xml SetColor(object/*dynamic*/ color)
        {
        ...
        public Dictionary<string, string> GetAttributes()
        {
            ...
            while (true)
            {
                ValueTuple<string, string> _ = _nativeXml.fz_dom_get_attribute(i);
            	string val = _.Item1;
                string attr = _.Item2;
